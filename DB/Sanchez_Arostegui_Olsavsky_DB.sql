...
AS BEGIN
	...
	DECLARE @ESTADOPREVIO VARCHAR(20)
	DECLARE @IDTURNO INT

	SELECT @ESTADOPREVIO = Estado from deleted
	SELECT @ESTADO = Estado, @IDTURNO = ID FROM INSERTED
	
	IF (@ESTADOPREVIO = 'Cancelado' AND @ESTADO = 'Cancelado') BEGIN
